...
| **ID**      | **Requirement**                                                                                                                                                                                                                                   | **Pri** |
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| **EXP-001** | **Weekly report = the submission unit (report-as-folder, reworked Meeting 3 — BUILT).** A user CREATES + names a report folder (the business week is **Monday–Sunday**, a label only) and adds line items into it; the **whole report is submitted as one unit** (its `draft`/returned items → submitted), not line by line. `expense_reports` is the live folder (name, owner, week bounds). Its status is the **DERIVED aggregate** of its items (never stored) and the alert count is aggregated on read. Each line item still carries its own status + `expense_date` (governing its pay cycle, EXP-009) + `amount_cad` — **money reads are item-level, so the folder is a pure grouping layer** (no double-count; #1/#12 untouched). | **M**   |
...
| **EXP-006** | A report with a **submitted item** enters the Pending Approval queue (folder queue, scoped); a Manager/Admin **approves, sends back, or rejects** the whole folder — a **bulk action over the folder's submitted/returned items** — or opens the workspace to review **per item** (both are available). A foreign item needing a manual FX rate is skipped by the bulk action (opened for the per-item FX dialog).                                                                            | **M**   |
| **EXP-007** | **Edit rights (owner + state, server-enforced §5).** While an item is **UNAPPROVED**, its **owner** may edit/delete it (a Manager/Admin in scope may too). Once **APPROVED**, only an **Admin or Super Admin** may correct it (a correction never alters a finalized pay run's frozen line). Approved items are never deleted (preserved). The owner-capable writes floor at `expenses:create` with service-level authorization.                                        | **M**   |
