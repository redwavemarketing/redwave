...
| **EXP-004** | **Kilometre log.** One km log per day **per rep**, with multiple stops; the user selects single trip (−30 km) or round trip (−60 km); billable km × rate (default $0.45) is the amount, **computed server-side**. When a Maps key is configured, each stop is a Places autocomplete and the route distance is **re-derived server-side** from the coordinates (the client value is ignored); otherwise the user enters the total distance manually and the server falls back to it. **Trip semantics (confirmed):** a SINGLE trip is measured over the stops exactly as entered (stop 1 → … → stop N); a ROUND trip is measured as the **closed loop** — the return leg to the first stop is **appended automatically** when deriving distance, so the rep enters only the outbound stops (if the first stop was already re-entered as the literal last stop, it is not appended again). The −30/−60 deduction is unchanged by trip measurement. In the manual (no-Maps) fallback the rep types the full driven distance themselves. | **M**   |
