/**
 * ExpenseEntryPage — submit a new weekly report (`expenses:create`). Loads the field configs (the dynamic
 * categories + receipt rule) and clients (optional tagging) before rendering the form. 403 → AccessDenied.
 */
...
import { useFieldConfigs } from '../api/useExpenses';
...
export default function ExpenseEntryPage() {
  ...
  if (!canCreate || isForbidden(configs.error)) {
    return <AccessDenied message="Submitting expenses requires the expenses create permission." />;
  }
  ...
  return (
    <div className={styles.page}>
      <PageHeader
        breadcrumbs={<Breadcrumbs items={[{ label: 'Expenses', href: '/expenses' }, { label: 'New report' }]} />}
        title="New expense report"
        subtitle="Add items for the week. The km amount is computed on submit."
      />
